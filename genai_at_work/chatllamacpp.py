...
class LlamaCppLocal(Singleton):
    ...
    def text_chat(self, user_prompt: str, system_prompt: str = None, history: list = [], options: dict = None):
        ...
        try:
            ...
            # Perform First Call
            response = self.llm.create_chat_completion(messages=messages,**options)
    ...
    def chat(self, user_prompt: str = None, system_prompt: str = None, history: list = None,
        ...
        if tools is not None:
            ...
            reply, messages = self.text_chat(user_prompt=user_prompt, system_prompt=system_prompt, history=history, options=options)
    ...
    def switch_model_dynamic(self, repo_id: str = None, filename: str = None, chat_format: str = None, tokenizer=None, cache_dir: str = None, options: dict = None):
        ...
        # unload
        self._release_model()
        self.model_file = hf_hub_download(repo_id=repo_id, filename=filename, cache_dir=cache_dir)
        print("downloaded model file: ", self.model_file)
        self.llm = Llama(model_path=self.model_file, **options)
    ...
    def api_calling_v2(self,
        ...
        reply_status = "ok"
        reply_text = reply
        reply_messages = self.add_messages(ai_prompt=reply_text, history=messages)
    ...
    def _test_function_call(self):
        response, messages = self.chat("what's time is now?", tools="Yes", options=CHAT_OPTIONS)
        print(response)
        return response
    ...
    def _test_mixed_chatting(self):
        ...
        # for i in range(10):
        ##test UI call 1
        reply_text, reply_messages, reply_status = llama.api_calling_v2(prompt="hello", system_prompt=system_prompt, history=[])
        print(reply_text)
